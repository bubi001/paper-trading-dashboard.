...
st.set_page_config(
    page_title="8-ETF Institutional Paper Trading Terminal", layout="wide"
)
...
if not ledger_df.empty:
  ...
  ledger_df["Quantity"] = pd.to_numeric(
      ledger_df["Quantity"], errors="coerce"
  ).fillna(0)
  ledger_df["Buy_Price"] = pd.to_numeric(
      ledger_df["Buy_Price"], errors="coerce"
  ).fillna(0)
  ledger_df["Total_Amount"] = pd.to_numeric(
      ledger_df["Total_Amount"], errors="coerce"
  ).fillna(0)
  ...
  if not buy_trades.empty:
    ...
    equities_deployed, daily_pnl, total_cost, liquidcase_cash = (
        0.0,
        0.0,
        0.0,
        INITIAL_CASH,
...
# Calculate Total P&L (Current Deployed Value minus Total Cost)
